...
const Cart = () => {
  ...
  const globalCartItems = useSelector((state) => state.cartData.cartItems);

  useEffect(() => {
    // Ensures `cartItems` is set after the component is mounted
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Review and manage the products in your cart before proceeding to checkout."
        />
        <meta name="keywords" content="cart, shop, checkout, ecommerce" />
              ...
              {cartItems?.length > 0 ? (
                cartItems?.map((product) => (
                  <tr key={product?.id} className="border-t">
                    ...
                    <td className="px-4 py-4">
                      Rs.{calculateSubtotal(product).toFixed(2)}
                    </td>
                    <td className="px-4 py-4">
